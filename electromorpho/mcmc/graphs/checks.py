...
from electromorpho.mcmc.graphs.state_space import RestrictionViolation, DAGState
...
def check_consistency(state: DAGState):
    ...
    for v in state.adj.nodes_iter():
        ...
        if len(state.adj.parents(v)) > state.fan_in:
            raise RestrictionViolation(
                'Fan in restriction violated. |Pa({0})| = {1} > {2}'.format(v, card_pa, state.fan_in))
